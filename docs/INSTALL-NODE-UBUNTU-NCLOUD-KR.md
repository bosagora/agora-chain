# 네이버 클라우드의 Ubuntu 에 BOSagora 노드 설치하기

이 문서는 [네이버 클라우드](https://www.ncloud.com)의 Ubuntu 서버에 노드를 설치하는 방법이 포함되어 있습니다.

## 1. 네이버 클라우드에 서버 생성하기

로그인한 후 우측 상단의 `콘솔` 버튼을 클릭하여, 콘솔화면으로 이동하여야 합니다.  
콘솔은 좌측에 메뉴가 있습니다. 이곳에서 사용자는 서버와 네트워크등의 장비들을 설치하고 설정할 수 있습니다.  
서버를 생성하는 순서는 다음과 같습니다.  

```
VPC 생성 -> Subnet 생성 -> 서버 생성 ->  방화벽 (ACG) 설정
```

### 1.1. VPC 생성하기

VPC는 독립적인 가상의 네트워크 공간으로 사용자의 설정에 따라 자유롭게 구성할 수 있는 공간을 의미합니다.  
좌측메뉴의 `Services` 를 클릭합니다. 그러면 화면 오른쪽 영역에 네이버클라우드에서 지원하는 다양한 서비스들을 보여줍니다.  
이 곳에서 `Networking` 그룹의 `VPC` 항목을 선택합니다.  
그러면 좌측에 `Recently Viewed` 항목에 `VPC` 가 추가됩니다. 이곳의 `VPC Manager` 를 선택합니다.  
좌측 상단의 `VPC 생성` 버튼을 클릭합니다.  
`VPC 이름` 항목에  `myvpc` 을 입력합니다.  
`IP 주소 범위` 항목에 `10.0.0.0/16` 을 입력합니다.   
`생성` 버튼을 클릭하면 VPC가 생성되기 시작합니다.  

### 1.2. Subnet 생성하기

좌측메누의 `Recently Viewed` / `VPC` / `Subnet Manager` 를 선택합니다.  
좌측 상단의 `Subnet 생성` 버튼을 클릭합니다.  
`Subnet 이름` 항목에 `mysubnet` 을 입력합니다.  
`VPC` 항목에 `myvpc` 을 선택합니다.  
`IP 주소 범위` 항목에 `10.0.0.0/16` 을 입력합니다.  
`Internet Gateway 전용 여부` 항목에 `Y (Public)` 를 선택합니다.  
`용도` 항목에 `일반` 을 선택합니다.  
`생성` 버튼을 클릭하면 Subnet이 생성되기 시작합니다.  

### 1.3. 서버 생성하기

좌측메뉴의 `Services` 를 클릭합니다. 그러면 화면 오른쪽 영역에 네이버클라우드에서 지원하는 다양한 서비스들이 나타납니다.  
이 곳에서 `Compute` 그룹의 `Server` 항목을 선택합니다.  
하단의 `서버 생성` 버튼을 클릭합니다.  
...
서버 이미지 이름의 `ubuntu-20.04` 의 우측의 `다음` 버튼을 클릭합니다.

#### 1.3.2 서버 설정

`VPC` 힝목에 `myvpc`를 선택합니다.  
`Subnet` 항목에 `mysubnet` 를 선택합니다.  
`스토리지 종류` 항목에 `SSD` 를 선택합니다.  
`서버 세대` 항목에 `g2` 를 선택합니다.  
`서버 타입` 항목에 `High CPU`, `CPU 2개, 메모리 4GB, SSD 50GB` 를 선택합니다.  
`요금제` 항목에 `시간 요금제` 를 선택합니다.  
`서버 개수` 항목에 `1` 을 입력합니다.  
`서버 이름` 항목에 `my-bosagora-validator` 를 입력합니다.  
`Network Interface` 항목에서 우측 `추가` 버튼을 눌러 네트워크 인터패이스를 추가합니다.  
`공인 IP` 항목에서 `새로운 공인 IP 할당` 를 선택합니다.  
하단의 `다음` 버튼을 클릭합니다.  

#### 1.3.3 인증키 설정

이미 인증키를 가지고 있다면 그것을 선택해도 됩니다.  
새로운 인증키를 만들기 위해서는 인증키의 이름을 입력하고 `인증키 생성 및 저장` 버튼을 클릭하면 인증키가 다운로드 됩니다.  
다운로드된 인증키를 잘 보관하여야 서버의 접속 비밀번호를 찾을 수 있습니다.   
`다음` 버튼을 클릭하여 계속 진행합니다.  

#### 1.3.4 네트워크 접근 설정

네트워크에 대한 접근허용에 대한 설정을 기록한 ACG를 선택합니다.  
VPC가 생성될 때 기본적으로 생성된 `myvpc-default-acg` 를 선택합니다.  
`다음` 버튼을 클릭하여 계속 진행합니다.  

#### 1.3.5 최종확인

지금까지 설정한 내용들을 보여줍니다. 하단의 `서버 생성` 버튼을 클릭합니다.  
그러면 Server 리스트가 나오는 화면으로 전환되고 몇분후에 서버생성이 완료됩니다.  
서버 생성이 완료되면 공인 IP가 생성됩니다.  

### 1.4. 방화벽 (ACG) 설정하기

좌측메뉴의 `Recently Viewed` 아래의 `Server` 를 선택합니다.  
좌측메뉴의 `ACG` 를 클릭합니다.  
서버에 설정한 ACG `myvpc-default-acg` 앞의 체크박스를 클릭합니다. 그러면 하단에 탭들이 나타납니다.  
위쪽의 `ACG 설정` 버튼을 클릭하면 편집화면이 팝업됩니다  

`Inbound` 탭을 선택합니다.   

`프로토콜`: `TCP`; `접근 소스` : `0.0.0.0/0`; `허용 포트` : `30303`; 를 입력합니다.   
`추가` 버튼을 클릭합니다.  

`프로토콜`: `UDP`; `접근 소스` : `0.0.0.0/0`; `허용 포트` : `30303`; 를 입력합니다.   
`추가` 버튼을 클릭합니다.  

`프로토콜`: `TCP`; `접근 소스` : `0.0.0.0/0`; `허용 포트` : `13000`; 를 입력합니다.   
`추가` 버튼을 클릭합니다.  

`프로토콜`: `UDP`; `접근 소스` : `0.0.0.0/0`; `허용 포트` : `12000`; 를 입력합니다.  
`추가` 버튼을 클릭합니다.   

`프로토콜`: `TCP`; `접근 소스` : `0.0.0.0/0`; `허용 포트` : `3000`; 를 입력합니다.  
`추가` 버튼을 클릭합니다.  
...
### 3.3. 네이버 클라우드 서버의 접속비밀번호 찾기

좌측메뉴의 `Recently Viewed` 아래의 `Server` 를 선택합니다.  
접속할 서버를 선택하고 바로 위의 `서버 관리 및 설정 변경` 버튼을 클릭하면 메뉴가 나타납니다.   
이 메뉴에서 `관리자 비밀번호 확인` 을 선택합니다.  그러면 관리자 비밀번호 확인 창이 팝업됩니다.  
이곳에 서버를 생성할 때 만들었던 `인증키` 를 `윈도우즈 탐색기` 에서 찾아서 드래그 한 후 해당 창에 드랍합니다.   
하단의 `비밀번호 확인` 버튼을 클릭합니다.  
그러면 비밀번호를 보여주는 창이 나타납니다.   
