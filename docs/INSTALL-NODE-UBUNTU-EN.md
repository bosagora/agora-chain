...
- Specify a key pair name, set the key pair type and private key file format (RSA, .pem), and click Generate Key Pair to
...
- When the launching of instance is complete, click the button `View all instances`, and then verify that the instance
  has been created.
...
Include the current account in the docker group so that the docker's commands can be used by accounts other than
the `root` account.
...
Executing the above command will receive the previous blocks. You can see that 'age' is getting smaller and smaller in
the log contents.  
If the block number in the log is the same as the most recent block number found at http://boascan.io, all historical
...
If the slot in the log is the same as the most recent slot found on http://agorascan.io, all historical data has been
received.
...
A folder called 'root/wallet' is created when the validator key is imported. Multiple validation keys are stored here as
a single file.  
You must also enter a password to access 'wallet'.  
When you import the first validator key, 'wallet' is automatically generated. Enter the password for 'wallet' at this
time.  
Therefore, you should understand that the password of the validator key and the password of the wallet may exist
differently.
...
If it is printed on the same day as above, it means that the validator key cannot be imported because the password is
entered incorrectly.
...
However, only the user knows the password for 'wallet', and the program does not know it, so program ask it every time
it runs.  
...
The information in the account can be set differently for each validator. For convenience, we will set the payment to
one address.
...
If the validator client does not start within 12 hours of deposit, the BOA's balance will be reduced due to a
penalty.   
...
The 'agora-el-node', 'agora-cl-node', and 'agora-cl-validator' that have been started so far will also terminate the
process when the connection to the server is stopped.  
...
When all blocks are synchronized, you press ctrl+c to end the process in this order: 'agora-cl-validator', '
agora-cl-node', and 'agora-el-node'.
...
Click the gear icon at the bottom of the left menu to display the menu window. Select `Data sources` from the menu
pane.  
...
When you click the fourth icon at the top of the left menu, a menu called `Dashboards` appears. Select `+ import` from
the menu pane.  
